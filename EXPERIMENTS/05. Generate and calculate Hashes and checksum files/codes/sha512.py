...
def _prepare_message(message: str) -> bytearray:
...
def _process_chunk(chunk: bytes, hash_values: list) -> list:
    w = [0] * 80
    ...
    return [(x + y) & 0xFFFFFFFFFFFFFFFF for x, y in zip(hash_values, (a, b, c, d, e, f, g, h))]

def sha512(message: str) -> str:
    if not isinstance(message, str):
        raise TypeError('Given message should be a string.')

    message_array = _prepare_message(message)
    ...
    for chunk_start in range(0, len(message_array), 128):
        ...
        hash_values = _process_chunk(chunk, hash_values)

    return binascii.hexlify(b''.join(struct.pack('!Q', value) for value in hash_values)).decode('utf-8')